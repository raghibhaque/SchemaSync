...
import { FileText, AlertCircle, CheckCircle2, Copy } from 'lucide-react'
import { parsePrismaSchema, validatePrismaSchema, getPrismaExampleSchema } from '../../lib/prismaParser'
...
export default function PrismaSchemaImporter({ onSchemaParsed }: Props) {
  ...
  const handleParse = () => {
    ...
    try {
      const tables = parsePrismaSchema(schemaContent)
      if (tables.length === 0) {
        setParseErrors(['No valid models found in schema'])
        setParsedTables([])
      } else {
        setParseErrors([])
  ...
  const handleLoadExample = () => {
    const example = getPrismaExampleSchema()
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-white/80">Prisma Schema Import</h3>
      </div>

      <div className="space-y-2">
        <label className="text-xs text-white/60">Paste your Prisma schema.prisma content:</label>

        <textarea
          value={schemaContent}
          onChange={(e) => setSchemaContent(e.target.value)}
          placeholder="Paste Prisma schema here..."
          className="w-full h-32 bg-white/[0.05] border border-white/[0.1] rounded px-3 py-2 text-xs font-mono text-white/70 placeholder:text-white/30 focus:outline-none focus:border-indigo-500/50"
        />
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 px-3 py-2 rounded text-xs font-medium bg-indigo-500/20 text-indigo-300 hover:bg-indigo-500/30 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            Parse Schema
          </button>
          <button
      ...
      {showExample && (
        <div className="bg-white/[0.03] border border-white/[0.08] rounded p-3 space-y-2">
          <div className="flex items-center justify-between">
            <p className="text-xs text-white/50">Example Prisma Schema:</p>
            <button
          ...
          <pre className="text-[10px] font-mono text-white/50 overflow-x-auto max-h-40 bg-black/20 rounded p-2">
            {getPrismaExampleSchema()}
          </pre>
          <button
            onClick={handleLoadExample}
            className="w-full text-xs px-2 py-1 rounded bg-indigo-500/20 text-indigo-300 hover:bg-indigo-500/30 transition-colors"
          >
            Load This Example
          </button>
        </div>
